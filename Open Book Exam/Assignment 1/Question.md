...
Model Selection: Choose two distinct model families that are well-suited for this classification task and adhere to the "single architecture" constraint (e.g., Family 1: Logistic Regression, Family 2: SVM).
Model Training & Hyperparameter Tuning: For each of the two model families, train the model on your prepared dataset.
Implement a hyperparameter tuning strategy (e.g., GridSearchCV, RandomizedSearchCV) to find the optimal set of parameters.
The primary optimization metric must be the Area Under the Precision-Recall Curve (PR-AUC). Justify why PR-AUC is a more suitable metric than ROC-AUC for this business problem 
Model Comparison: Present the best PR-AUC scores for each of the two tuned models. Select the single best-performing model that you will recommend to the stakeholders.
